...
const Modal = ({ selectedAppointment, onAppointmentUpdated }: ModalProps = {} as ModalProps) => {
  ...
  useEffect(() => {
    const loadViewAppointment = async () => {
      if (selectedAppointment?._id || selectedAppointment?.id) {
        try {
          const appt = await fetchAppointmentById(selectedAppointment._id || selectedAppointment.id || "");
          setViewAppointment(appt);
        } catch (err) {
          // eslint-disable-next-line no-console
          console.error("Failed to load appointment:", err);
          setViewAppointment(selectedAppointment);
        }
      }
    };
    loadViewAppointment();
  ...
  const handleDeleteAppointment = async () => {
    ...
    if (!window.confirm(`Are you sure you want to delete appointment for ${selectedAppointment.Patient}?`)) {
      return;
    }
    ...
    try {
      ...
      alert(`Failed to delete appointment: ${error?.response?.data?.message || error?.message || "Unknown error"}`);
    } finally {
      setLoading(false);
    }
  };
